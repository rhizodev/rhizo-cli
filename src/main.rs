...
fn print_help(){
    ...
    println!("ls [socb | route]\t\t\t\tFetch the list of the developer's hosted routes");   
    println!("socb [alloc | write] $key $content_path\t\tWrite or allocate signed-onchain bytes for the current keypair");
    ...
    println!("view [socb | route] $key\t\t\tFetch the configuration for the provided route");
    println!("test $wasm_module_path\t\t\t\tExecute the module's test() function locally. Useful for local testing and finding runtime errors.");
    ...
    match args.get(1) {
        Some(command) => {
            ...
            if command.to_lowercase().eq("yank"){
                ...
                if ocb_cmd.eq("write"){
                   println!("ocb write {:?} {:?}", ocb_seed, ocb_content);
                    match ocb_write(ocb_seed, ocb_content.as_bytes().to_vec()) {
                        Err(e) => println!("[ERROR] ocb set failed due to: {}", e),
                        Ok(_) => {}
                    } 
                } else if ocb_cmd.eq("alloc") {
                    println!("ocb alloc {:?}", ocb_seed);
